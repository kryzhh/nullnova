...
def wipe_device(device_path, size, method, chunk_size=CHUNK_SIZE, gui=None):
    ...
    if method.startswith("DoD"):
        patterns = [0x00, 0xFF, None]  # zeros, ones, random
        passes = 3
    else:
        patterns = ["crypto"]
        passes = 2
    ...
    for p in range(passes):
        for c in range(chunks):
            to_write = min(chunk_size, size - (c * chunk_size))
            if patterns[p % len(patterns)] == "crypto":
                crypto_wipe(handle, to_write)
            else:
                write_pattern(handle, to_write, patterns[p % len(patterns)])

            done += 1
            progress = (done / (chunks * passes)) * 100
            if gui:
                gui.update_progress(progress, f"Pass {p+1}/{passes}, Chunk {c+1}/{chunks}")
